...
frappe.views.excel.FieldPicker = class FieldPicker {
	...
	_build_fields() {
		...
		// ── Sort: all non-locked fields alphabetically by label ─────────────────
	...
	_item_html(f) {
		...
		return `
			<li class="ev-fp-item${f.locked ? " ev-fp-locked" : ""}"
				data-fieldname="${frappe.utils.escape_html(f.fieldname)}"
				draggable="${f.locked ? "false" : "true"}"
				role="option">
				<span class="${drag_cls}" aria-hidden="true">⠿</span>
	...
	_bind_events($body) {
		...
		$body.on("input", ".ev-fp-search", (e) => {
			...
			this.$list[0].querySelectorAll(".ev-fp-item").forEach(el => {
				...
				el.hidden = !!(q && !label.includes(q) && !fn.includes(q));
			});
			this._update_count();
		});
	...
	_on_apply() {
		...
		const selected = [];
		this.$list[0].querySelectorAll(".ev-fp-item").forEach(el => {
			if (el.querySelector(".ev-fp-check").checked) {
				selected.push(el.dataset.fieldname);
			}
		});
		...
		// Persist server-side (per-user per-doctype, survives browser clear)
		frappe.model.user_settings.save(this.doctype, "excel_columns", selected);

		this._dialog.hide();
		this.board.apply_field_selection(selected);
	}
};
